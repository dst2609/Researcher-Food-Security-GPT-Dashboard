...
const DebtServiceTotal = () => {
  const [countries, setCountries] = useState([]);
  const [selectedCountry, setSelectedCountry] = useState('World');
  const [chartData, setChartData] = useState([["Year", "Total Reserves"]]);

  useEffect(() => {
    //API_DT.TDS.DECT.GN.ZS_DS2_en_csv_v2_58635/API_DT.TDS.DECT.GN.ZS_DS2_en_csv_v2_58635.csv
    const csvFileUrl = "./API_DT.TDS.DECT.GN.ZS_DS2_en_csv_v2_58635/API_DT.TDS.DECT.GN.ZS_DS2_en_csv_v2_58635.csv";
    ...
    <div>
      ...
      {selectedCountry !== 'World' && (
        <Chart
          ...
          data={chartData}
          options={{
            title: `Total debt service (% of GNI) for ${selectedCountry}`,
            hAxis: { title: 'Year' },
            vAxis: { title: 'Total debt service (% of GNI)' },
          }}
        />
      )}
    </div>
